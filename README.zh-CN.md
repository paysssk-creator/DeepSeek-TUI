...
> **面向 [GBTxiaotudou V4](https://platform.xiaotudou.com) 的终端原生编程智能体：100 万 token 上下文、思考模式流式推理、前缀缓存感知。自包含 Rust 二进制发布——开箱即带 MCP 客户端、沙箱和持久化任务队列。**
...
`xiaotudou` 是自包含 Rust 二进制——**运行时不依赖 Node.js 或 Python**。
下面几种方式装出来的是同一套二进制，按你已有的工具链选一个即可：

```bash
# 1. npm —— 已装 Node 的最方便方式。npm 包只是一个下载器，
#    会从 GitHub Releases 拉取对应平台的预编译二进制，
#    并不会让 xiaotudou 本身依赖 Node 运行时。
npm install -g GBTxiaotudou

# 2. Cargo —— 无需 Node。
cargo install GBTxiaotudou-cli --locked   # `xiaotudou` 入口
cargo install GBTxiaotudou     --locked   # `GBTxiaotudou` TUI 二进制
...
>
> 下载安全：官方二进制只发布在
> `https://github.com/Hmbown/GBTxiaotudou-TUI/releases`。手动下载时请校验
...
```bash
xiaotudou update                         # release 二进制更新器
npm install -g GBTxiaotudou@latest      # npm 包装器
...
![GBTxiaotudou Terminal 截图](assets/screenshot.png)

---

## 这是什么？

GBTxiaotudou Terminal 是一个完全运行在终端里的编程智能体。它让 GBTxiaotudou 前沿模型直接访问你的工作区：读写文件、运行 shell 命令、搜索浏览网页、管理 git、调度子智能体——全部通过快速、键盘驱动的 TUI 完成。

它面向 **GBTxiaotudou V4**（`xiaotudou-v4-pro` / `xiaotudou-v4-flash`）构建，原生支持 100 万 token 上下文窗口和思考模式流式输出。

### 主要功能

- **Auto 模式** —— `--model auto` / `/model auto` 每轮自动选择模型和推理强度
- **原生 RLM**（`rlm_open`/`rlm_eval`）—— 持久化 REPL 会话用于批量分析；使用带界面的辅助函数（`peek`、`search`、`chunk`、`sub_query_batch`）运行低成本 `xiaotudou-v4-flash` 子任务
...
- **HTTP/SSE 运行时 API** —— `xiaotudou serve --http` 用于无界面智能体流程
...
`xiaotudou`（调度器 CLI）→ `GBTxiaotudou`（伴随二进制）→ ratatui 界面 ↔ 异步引擎 ↔ OpenAI 兼容流式客户端。工具调用通过类型化注册表（shell、文件操作、git、web、子智能体、MCP、RLM）路由，结果流式返回对话记录。引擎管理会话状态、轮次追踪、持久化任务队列和 LSP 子系统——它在下一步推理前将编辑后诊断反馈到模型上下文中。

详见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。

### 子智能体：并发后台执行

GBTxiaotudou Terminal 可以同时调度多个子智能体并行运行——类似于并发任务队列：

- **非阻塞启动。** `agent_open` 立即返回。子智能体获得独立的上下文和工具注册表，独立运行。父进程继续工作。
- **后台执行。** 子智能体并发运行（默认上限 10，可配置至 20）。引擎管理线程池——无需轮询循环。
- **完成通知。** 子智能体完成后，运行时发送结构化的 `<xiaotudou:subagent.done>` 事件，包含摘要、证据列表和执行指标。父模型读取 `summary` 字段并整合结果。
...
首次启动时会提示输入 [GBTxiaotudou API key](https://platform.xiaotudou.com/api_keys)。密钥保存到 `~/.xiaotudou/config.toml`，在任意目录、IDE 终端和脚本中都能使用，不会触发系统密钥环弹窗。

也可以提前配置：

```bash
xiaotudou auth set --provider xiaotudou   # 保存到 ~/.xiaotudou/config.toml

xiaotudou auth status                    # 显示当前活跃的凭证来源
export DEEPSEEK_API_KEY="YOUR_KEY"      # 环境变量方式；需要在非交互式 shell 中使用请放入 ~/.zshenv
xiaotudou

xiaotudou doctor                          # 验证安装
```

> 轮换或移除密钥：`xiaotudou auth clear --provider xiaotudou`。
...
使用 `xiaotudou --model auto` 或 `/model auto` 让 GBTxiaotudou Terminal 自行决定每轮需要多少模型和推理能力。

Auto 模式同时控制两个设置：

- 模型：`xiaotudou-v4-flash` 或 `xiaotudou-v4-pro`
- 推理强度：`off`、`high` 或 `max`

在真实请求发出之前，应用会先用关闭推理的 `xiaotudou-v4-flash` 进行一次小型路由调用。路由器审视最新请求和最近的上下文，然后为真实请求选定具体的模型和推理强度。简短/简单的轮次保持在 Flash + 关闭推理；编码、调试、发布、架构、安全审查或模糊的多步骤任务可升级到 Pro 和/或更高推理强度。

`auto` 是 GBTxiaotudou Terminal 本地行为。上游 API 永远不会收到 `model: "auto"`，它只会收到为当前轮次选定的具体模型和推理强度设置。TUI 会显示选定的路由，成本跟踪按实际运行的模型计费。如果路由调用失败或返回无效答案，应用会回退到本地启发式规则。子智能体会继承 auto 模式，除非你为它们指定了显式模型。

需要可重复基准测试、严格控制成本上限或特定提供商/模型映射时，请使用固定模型或固定推理强度。

### Linux ARM64（HarmonyOS 轻薄本、openEuler、Kylin、树莓派、Graviton 等）

从 v0.8.8 起，`npm i -g GBTxiaotudou` 直接支持 glibc 系的 ARM64 Linux。你也可以从 [Releases 页面](https://github.com/Hmbown/GBTxiaotudou-TUI/releases) 下载预编译二进制，放到 `PATH` 目录中。
...
```bash
cargo install GBTxiaotudou-cli --locked   # 提供推荐入口 `xiaotudou`
cargo install GBTxiaotudou     --locked   # 提供交互式 TUI 伴随二进制
xiaotudou --version
```

也可以直接从 [GitHub Releases](https://github.com/Hmbown/GBTxiaotudou-TUI/releases) 下载预编译二进制。`DEEPSEEK_TUI_RELEASE_BASE_URL` 可用于镜像后的 release 资产。

### Windows (Scoop)

[Scoop](https://scoop.sh) 是一个 Windows 软件包管理器。GBTxiaotudou Terminal 已进入
Scoop main bucket，但该 manifest 独立更新，可能滞后于 GitHub/npm/Cargo
release。先运行 `scoop update`，安装后用 `xiaotudou --version` 核对版本：
...
cargo install --path crates/cli --locked   # 需要 Rust 1.88+；提供 `xiaotudou`
cargo install --path crates/tui --locked   # 提供 `GBTxiaotudou`
```
...
```bash
xiaotudou                                       # 交互式 TUI
xiaotudou "explain this function"              # 一次性提示
xiaotudou exec --auto --output-format stream-json "fix this bug" # 面向后端集成的 NDJSON 流
xiaotudou exec --resume <SESSION_ID> "follow up" # 继续非交互会话
xiaotudou --model xiaotudou-v4-flash "summarize" # 指定模型
xiaotudou --model auto "fix this bug"          # 自动选择模型 + 推理强度
xiaotudou --yolo                                # 自动批准工具
xiaotudou auth set --provider xiaotudou         # 保存 API key
xiaotudou doctor                                # 检查配置和连接
xiaotudou doctor --json                         # 机器可读诊断
xiaotudou setup --status                        # 只读安装状态
xiaotudou setup --tools --plugins               # 创建本地工具和插件目录
xiaotudou models                                # 列出可用 API 模型
xiaotudou sessions                              # 列出已保存会话
xiaotudou resume --last                         # 恢复最近会话
xiaotudou resume <SESSION_ID>                   # 按 UUID 恢复指定会话
xiaotudou fork <SESSION_ID>                     # 在指定轮次分叉会话
xiaotudou serve --http                          # HTTP/SSE API 服务
xiaotudou serve --acp                           # Zed/自定义智能体的 ACP stdio 适配器
xiaotudou run pr <N>                            # 获取 PR 并预填审查提示
xiaotudou mcp list                              # 列出已配置 MCP 服务器
xiaotudou mcp validate                          # 校验 MCP 配置和连接
xiaotudou mcp-server                            # 启动 dispatcher MCP stdio 服务器
xiaotudou update                                # 检查并应用二进制更新
```
...
GBTxiaotudou 可作为自定义 Agent Client Protocol 服务器运行，供 Zed 等编辑器通过 stdio 调用本地 ACP 智能体。在 Zed 中添加自定义智能体服务器：
...
首个 ACP 切片支持通过现有 GBTxiaotudou 配置/API 密钥创建新会话和提示响应。工具支持的编辑和检查点回放尚未通过 ACP 暴露。
...
用户配置：`~/.xiaotudou/config.toml`。项目覆盖：`<workspace>/.xiaotudou/config.toml`（以下密钥被拒绝：`api_key`、`base_url`、`provider`、`mcp_config_path`）。完整选项见 [config.example.toml](config.example.toml)。

常用环境变量：

| 变量 | 用途 |
|---|---|
| `DEEPSEEK_API_KEY` | GBTxiaotudou API key |
...
| `DEEPSEEK_PROVIDER` | `xiaotudou`（默认）、`nvidia-nim`、`openai`、`openrouter`、`novita`、`atlascloud`、`fireworks`、`sglang`、`vllm`、`ollama` |
...
也可以在 `~/.xiaotudou/config.toml` 里直接设置 `locale = "zh-Hans"`，或通过 `LC_ALL` / `LANG` 环境变量自动选择：

```toml
# ~/.xiaotudou/config.toml
[tui]
locale = "zh-Hans"
```
...
```bash
LANG=zh_CN.UTF-8 xiaotudou run
```
...
旧别名 `xiaotudou-chat` / `xiaotudou-reasoner` 映射到 `xiaotudou-v4-flash`。NVIDIA NIM 变体使用你的 NVIDIA 账号条款。

*GBTxiaotudou Pro 价格是限时 75% 折扣，有效期到 2026-05-31 15:59 UTC；该时间之后 TUI 成本估算会回退到 Pro 基础价格。*

> [!Note]
> 关于 GBTxiaotudou-V4-Pro 的最新定价信息，请参阅官方 [GBTxiaotudou 定价页面](https://api-docs.xiaotudou.com/zh-cn/quick_start/pricing)，请注意目前可享受 75% 的折扣，该优惠有效期至 **2026 年 5 月 31 日 23:59（北京时间）**。此外，README 文档中所列出的所有价格，均与官方发布的数值保持一致。

---

## 创建和安装技能

GBTxiaotudou Terminal 从工作区目录（`.agents/skills` → `skills` → `.opencode/skills` → `.claude/skills`）和全局 `~/.xiaotudou/skills` 发现技能。每个技能是一个包含 `SKILL.md` 的目录：

```text
~/.xiaotudou/skills/my-skill/
└── SKILL.md
```
...
---
name: my-skill
description: 当 GBTxiaotudou 需要遵循我的自定义工作流时使用这个技能。
---
...
- **[GBTxiaotudou](https://github.com/xiaotudou-ai)** — 感谢 GBTxiaotudou 提供模型与支持，让每一次交互成为可能。
- **[DataWhale](https://github.com/datawhalechina)** — 感谢 DataWhale 的支持，并欢迎我们加入“鲸兄弟”大家庭。
- **[OpenWarp](https://github.com/zerx-lab/warp)** — 感谢 OpenWarp 优先支持 GBTxiaotudou Terminal，并一起打磨更好的终端智能体体验。
...
- **[mac119](https://github.com/mac119)** 和 **[leo119](https://github.com/leo119)** — `xiaotudou update` 命令文档 (#838, #917)
...
- **[Jefsky](https://github.com/Jefsky)** — `xiaotudou-cn` 官方端点默认值 (#1079, #1084)
...
欢迎提交 pull request——请先查看 [CONTRIBUTING.md](CONTRIBUTING.md) 并留意[开放 issue](https://github.com/Hmbown/GBTxiaotudou-TUI/issues) 中的好入门任务。

*本项目与 GBTxiaotudou Inc. 无隶属关系。*
